...
function initGame()
{
	...
	if (inited != currentGame)
	{
		inited = currentGame;
		var squares = [];
		// squaresMap.set(currentGame,squares);
		// Stage
		canvas = document.getElementById("gameContainer1");  // initiatedCount
	...
	// This is a workaround to avoid multiple listeners to be added when a game
	// is left and a new one is joined
	// TrisGame should be a separate class, instantiated each time a new game is
	// started
	...
	console.log("Before ready - sfs.lastJoinedRoom- "+sfs.lastJoinedRoom);
...
// var squares = null;
// if(squaresMap.has(currentGame))
// {
// squares = squaresMap.get(currentGame);
// }
	// Background
...
function startGame(params)
{
	...
	lastJoinFirst = sfs.lastJoinedRoom;  // Added
	...
	enableBoard(true);
// }
// else if(squaresMap.size ==2)
// {
// enableBoardSecondary(true);
// }
...
{
	if(iAmSpectator == false){
		statusTF.text = (sfs.mySelf.getPlayerId(lastJoinFirst) == whoseTurn) ? "It's your turn" : "It's your opponent's turn";  //sfs.lastJoinedRoom
	}else{
 ...
 * Enable board click
 */
function enableBoardSecondary(enable)
{
// var squares= null;
// if(squaresMap.has(currentGame))
// {
// squares = squaresMap.get(currentGame);
// }
	console.log("##### enableBoardSecondary sfs.mySelf.Name "+sfs.mySelf.Name + "  lastJoinFirst- "+lastJoinFirst + "   whoseTurn- "+whoseTurn+ "  sfs.mySelf.getPlayerId(lastJoinFirst)- "+sfs.mySelf.getPlayerId(lastJoinFirst));
	if(iAmSpectator == false && sfs.mySelf.getPlayerId(lastJoinFirst) == whoseTurn)  //  sfs.lastJoinedRoom
	{
		for(var i = 0; i<9; i++){
...
// squares = squaresMap.get(currentGame);
// }
	console.log("##### enableBoard sfs.mySelf.Name "+sfs.mySelf.name + "  lastJoinFirst- "+lastJoinFirst + "   whoseTurn- "+whoseTurn+ "  sfs.mySelf.getPlayerId(lastJoinFirst)- "+sfs.mySelf.getPlayerId(lastJoinFirst));
	if(iAmSpectator == false && sfs.mySelf.getPlayerId(lastJoinFirst) == whoseTurn) //    sfs.lastJoinedRoom
	{
		for(var i = 0; i<9; i++){
			var square = squares[i];
...
function makeMove(evt)
{
	...
	square.ball.gotoAndStop(sfs.mySelf.getPlayerId(lastJoinFirst));  //   lastJoinFirst   sfs.lastJoinedRoom
	console.log(" make move first - "+lastJoinFirst + " sfs.mySelf.getPlayerId(lastJoinFirst) - "+sfs.mySelf.getPlayerId(lastJoinFirst));
	square.onClick = null;
	//sfs.lastJoinedRoom = lastJoinFirst;
	enableBoard(false);
	...
	sfs.send( new SFS2X.ExtensionRequest("move", obj,lastJoinFirst) )
}
...
function moveReceived(params)
{
	...
	console.log("moveReceived first sfs.mySelf.getPlayerId(lastJoinFirst) - "+sfs.mySelf.getPlayerId(lastJoinFirst));
// var squares= null;
// if(squaresMap.has(currentGame))
// {
// squares = squaresMap.get(currentGame);
// }
//	lastJoinFirst = sfs.lastJoinedRoom;
	if(movingPlayer != sfs.mySelf.getPlayerId(lastJoinFirst))
	{
		console.log("movingPlayer - "+movingPlayer +" and whoseTurn- "+whoseTurn+ " lastJoinFirst- "+lastJoinFirst);
...
function restartGame()
{
	...
	sfs.send( new SFS2X.ExtensionRequest("restart", null, lastJoinFirst) )  //   sfs.lastJoinedRoom
}
...
function showGamePopUp(id, message)
{
	...
	currentPopUp = $("#"+id+"GameWin1");  //+initiatedCount
...
function onExtensionResponse(evt)
{
	...
	var cmd = evt.cmd;
	console.log(" params.room.game - "+evt.room.name);
	
	if(firstGame == evt.room.name)
	{
		console.log(">First Received Extension Response: " + cmd);
	switch(cmd)
	{
