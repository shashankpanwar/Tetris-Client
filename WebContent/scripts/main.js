...
// Constants
//------------------------------------
var LOBBY_ROOM_NAME = "Tris Lobby";
var GAME_ROOMS_GROUP_NAME = "trisGames";
...
var EXTENSION_ID = "TrisExtension";
...
var lastJoinFirst=null;
var lastJoinSecond=null;
...
function init()
{
	...
	sfs.addEventListener(SFS2X.SFSEvent.ROOM_JOIN_ERROR, onRoomCreationError, this);
...
/**
 * Connect button click handler. Connects to a SFS2X instance.
 */
...
/**
 * Login button click handler. Performs the login, which in turn (see onLogin
 * event) makes the view switch to the lobby.
 */
function onLoginBtClick()
{
	...
	var uPass = $("#passwordIn").val();
	var params = new SFS2X.SFSObject();
	params.putUtfString("password", uPass);
	var isSent = sfs.send(new SFS2X.LoginRequest(uName,"",params));
...
/**
 * Disconnect button click handler. Disconnects the client from the SFS2X
 * instance.
 */
...
/**
 * Logout button click handler. Performs the logout, which in turn (see onLogout
 * event) makes the view switch to the login box.
 */
...
/**
 * Public message send button click handler. Send a public message, which will
 * be displayed in the chat area (see onPublicMessage event).
 */
...
/**
 * When a user is selected in the user list, enable the button to challenge him
 * in a new game. In the game creation window the game is automatically set to
 * private and the user is selected in the invitees list.
 */
function onUserSelected(event)
{
	...
	// Only consider user selection made using mouse or keyboard (API call is
	// excluded)
	if (selectionType != "none")
	{
		...
		// For example code simplicity we rebuild the full userlist instead of
		// just editing the specific item
...
/**
 * When a room is selected in the room list, the "Play" and "Watch" buttons are
 * enabled
 */
...
function onGameRoomSelected(event)
{
	var doEnable = event != undefined;
	...
	{
		selectedRoom = null;
		$("#roomListGame").jqxListBox("clearSelection");
	}

	enableButton("#playGameBtInside", doEnable);
	enableButton("#watchGameBtInside", doEnable);
	enableButton("#deselectGameBtInside", doEnable);
}
...
function onPlayGameBtInsideClick(event)
{
	if ($("#roomListGame").jqxListBox("selectedIndex") > -1)
	{
		// Join selected room
		var room = $("#roomListGame").jqxListBox("getSelectedItem").originalItem.roomObj;
		sfs.send(new SFS2X.JoinRoomRequest(room,"",-1,false));
	}
}
...
/**
 * Leave game button click handler. In order to leave the current game room, the
 * lobby room is joined.
 */
...
/**
 * Create game button click handler. Shows the game room creation panel.
 */
...
/**
 * When the game creation panel is closed, all the form items it contains are
...
/**
 * Create game button click event listener (create game panel). Create a new
 * game using the parameters entered in the game creation popup window.
 */
...
function onLogin(event)
{
	...
	// Set user name
	// NOTE: this always a good practice, in case a custom login procedure on
	// the server side modified the username
...
function onRoomJoin(event)
{
	...
	else
	{
		...
		currentGame=event.room.name;
		// Initialize the game
		if (inGame && !inGameSecond)
		{
			firstGame = event.room.name;
			initGame(event);
			inGameSecond=true;
		}
		else if(inGameSecond)
		{
			secondGame = event.room.name;
			initGameSecond(event);
		}
...
function onUserExitRoom(event)
{
	if (event.room.name == LOBBY_ROOM_NAME)
	{
		...
		// For example code simplicity we rebuild the full userlist instead of
		// just removing the specific item
		populateUsersList();
	}
	else
	{
...
{
	// For example code simplicity we rebuild the full roomlist instead of just
	// updating the specific item
	populateRoomsList();
	populateGameRoomsList();
}
...
function switchView(viewId)
{
	...
	$('.viewStack').each(function(index)
	{
		...
		{
			$(this).show();
			$(this).css({opacity:1}); // Opacity attribute is used on page
										// load to hide the views because
										// display:none causes issues to the
...
function populateGameRoomsList()
{
	var rooms = sfs.roomManager.getRoomList();
	var source = [];

	var selectedRoom = ($("#roomListGame").jqxListBox("selectedIndex") > -1 ? $("#roomListGame").jqxListBox("getSelectedItem").originalItem.roomObj.id : null);
	var selectedIndex;
	var index = 0;
	...
	$("#roomListGame").jqxListBox({source: source});
	
	// Set selected index
	$("#roomListGame").jqxListBox("selectedIndex", selectedIndex);
}
...
function populateUsersList()
{
	...
	// "main" indicates the main user list contained in the right accordion of
	// the lobby view
...
{
	trace("Room Creation error: " + event.errorMessage + " (code: " + event.errorCode + ")", true);
}
function onWatchGameBtInsideClick(event)
{
	console.log("onWatchGameBtInsideClick()");
}
function onDeselectGameInsideBtClick(event)
{
	console.log("onDeselectGameInsideBtClick()");
}